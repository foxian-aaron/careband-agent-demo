# 智护环 CareBand Agent Demo v0.1.1

智护环 CareBand Agent 是一个面向长者照护场景的 AI Agent 状态观察系统。v0.1.1 继续保持 Web Demo + 模拟数据 + 规则引擎 + Mock AI Agent，不接真实硬件、医院系统、数据库或真实大模型 API。
...
3. 在 Decision Trace 中展示输入数据、个人基线比较、触发规则、风险输出和三端摘要。
4. 打开 `#/demo-control`，点击“触发陈伯头晕语音事件”。
5. 陈伯升级为“高风险待处理”，护工端生成高优先级任务“陈伯需要立即查看”。
6. 在 `#/caregiver` 或控制台依次点击“护工接单”、“标记已查看”、“确认晚药”、“完成护工处理”。
7. 回到 `#/family/E001`，家属端主标题变为“陈伯今日状态：已跟进 / 持续观察”。
8. 回到 `#/institution`，机构端显示陈伯前台状态已跟进，同时保留“今日曾出现高风险事件”的说明；此时“当前未闭环高风险”为 0，“今日曾高风险”为 1，“已跟进高风险”为 1。
